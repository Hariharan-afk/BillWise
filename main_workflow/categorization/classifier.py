...
def llm_fallback_gemini(
    ...
    """
    Call Gemini to verify or reclassify an item when DistilBERT confidence
    falls in the medium tier (ROUTE_LLM).
    Uses the modern google-genai SDK.
    """
    try:
        from google import genai
    except Exception as exc:
        logging.error("Categorizer: google-genai import failed for '%s' - %s", item_text, exc)
        return {
            "final_label": classification["predicted_label"],
            "confidence": "low",
            "reason": f"Gemini SDK import failed: {exc}",
    ...
    prompt = f"""You are a grocery or food item categorization expert.

A receipt scanner returned this item text:
  Original  : "{item_text}"
  Normalized: "{normalized_text}"

A DistilBERT classifier predicted "{classification['predicted_label']}" with confidence {classification['confidence']:.4f} (medium - needs verification).
...
Choose the most appropriate category. If you are genuinely uncertain, set confidence to "low" so a human reviewer can be notified.
