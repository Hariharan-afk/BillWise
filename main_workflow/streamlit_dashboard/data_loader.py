...
from main_workflow.streamlit_dashboard.utils import categorize_item
...
_env_local_csv = os.environ.get("LOCAL_BILLS_CSV", "").strip()
if _env_local_csv:
    LOCAL_CSV = Path(_env_local_csv)
    if not LOCAL_CSV.is_absolute():
        LOCAL_CSV = (REPO_ROOT / LOCAL_CSV).resolve()
else:
    LOCAL_CSV = REPO_ROOT / "main_workflow" / "data" / "dev" / "bills_output.csv"
...
def _load_from_gcs() -> pd.DataFrame | None:
    ...
    if not _should_use_gcs() or not bucket_name:
        return None
    try:
...
def init_db() -> None:
    ...
    gcs_configured = _should_use_gcs()
...
def get_data_source_label() -> str:
    ...
    if _should_use_gcs() and _load_from_gcs() is not None:
